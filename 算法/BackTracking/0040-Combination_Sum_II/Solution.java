// Time: 3 ms (82.44%), Space: 43 MB (85.22%)
    ...
    private void backtrack(List<List<Integer>> answer, List<Integer> tempList, int[] candidates, int totalLeft, int index) 
    {
        if (totalLeft == 0) { // Add to the answer array, if the sum is equal to target.
            ...
            for (int i = index; i < candidates.length && totalLeft >= candidates[i]; i++) {
                ...
                //1. tempList initial start will always be unique
                //2. when in the middle, the next start will always be unique as well
                // - for[1, 2, 2, 2, 5] if index == 1, the next possible index is 4, so won't have multiple temp lists as [1, 2]
